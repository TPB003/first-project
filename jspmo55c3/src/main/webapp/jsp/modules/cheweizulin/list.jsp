...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">车位租赁管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>车位租赁管理</span></li>
								<li class="breadcrumb-item active"><span>车位租赁列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">车位租赁列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									<label>
										车位名称
									</label>
									<input type="text" id="cheweimingchengSearch" class="form-control form-control-sm" placeholder="请输入车位名称" aria-controls="tableId">
									<label>
										租赁类型
									</label>
									<select name="zulinleixing" id="zulinleixingSelect" class="form-control form-control-sm" aria-controls="tableId">
										<option value=" "> </option>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('cheweimingcheng')">车位名称<i id="cheweimingchengIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('cheweileixing')">车位类型<i id="cheweileixingIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('weizhi')">位置<i id="weizhiIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('zulinleixing')">租赁类型<i id="zulinleixingIcon" class="fa fa-sort"></i></th>
											...
											<th onclick="sort('loudongdanyuan')">楼栋单元<i id="loudongdanyuanIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#cheweimingchengSearch').val() != null && $('#cheweimingchengSearch').val() != ''){
				searchForm.cheweimingcheng ="%" +  $('#cheweimingchengSearch').val() + "%";
			}
			
			var zulinleixingIndex = document.getElementById("zulinleixingSelect").selectedIndex;
			var zulinleixingOptions = document.getElementById("zulinleixingSelect").options;
			var zulinleixingSelectedOption = zulinleixingOptions[zulinleixingIndex]
			if(zulinleixingSelectedOption.text != null && zulinleixingSelectedOption.text != ''){
				searchForm.zulinleixing = "%" + zulinleixingSelectedOption.text + "%";
			}
        ...
        function getDataList() {
			http("cheweizulin/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
				cheweimingcheng : searchForm.cheweimingcheng,	
				zulinleixing : searchForm.zulinleixing,	
		...
		function setDataRow(item,number){
			...
			var cheweimingchengCell  = document.createElement('td');

						 cheweimingchengCell.innerHTML = item.cheweimingcheng;

			row.appendChild(cheweimingchengCell);
			var cheweileixingCell  = document.createElement('td');

						 cheweileixingCell.innerHTML = item.cheweileixing;

			row.appendChild(cheweileixingCell);
			var weizhiCell  = document.createElement('td');

						 weizhiCell.innerHTML = item.weizhi;

			row.appendChild(weizhiCell);
			var zulinleixingCell  = document.createElement('td');

						  zulinleixingCell.innerHTML = item.zulinleixing; 

			row.appendChild(zulinleixingCell);
			...
			var loudongdanyuanCell  = document.createElement('td');

						 loudongdanyuanCell.innerHTML = item.loudongdanyuan;

			row.appendChild(loudongdanyuanCell);
		...
		function shDialog(id){
			$("#shId").attr("value",id);
			http("cheweizulin/info/" + id,"GET",{},(res)=>{
				if(res.code == 0){
		...
		function sh(){
			...
			httpJson("cheweizulin/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("cheweizulin/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"cheweizulin");
        ...
        function pay(item){
			...
			window.sessionStorage.setItem('tableName',"cheweizulin");
